...
<div
    ...
    <div class="container mx-auto p-6">
        <div class="max-w-4xl mx-auto bg-white rounded shadow-sm p-6">
            <h2 class="text-2xl font-semibold mb-6">Edit Ekstrakulikuler</h2>
            @if ($errors->any())
            ...
            @endif
            <form action="{{ route('extrakulikuler.update', $ex->id) }}" method="POST" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                <div class="grid grid-cols-12 gap-6">
                    <div class="col-span-12 md:col-span-4">
                        ...
                        <select name="s_menu_id" class="block w-full rounded border-gray-200 shadow-sm p-3">
                            ...
                            @if (!empty($menus))
                                @foreach ($menus as $m)
                                    <option value="{{ $m->id }}" {{ $ex->s_menu_id == $m->id ? 'selected' : '' }}>
                                        {{ $m->name }}</option>
                                @endforeach
                            @else
                    ...
                    <div class="col-span-12 md:col-span-8">
                        ...
                        <input type="text" name="name" value="{{ old('name', $ex->name) }}"
                            class="block w-full rounded border-gray-200 shadow-sm p-3" placeholder="Nama ekstrakulikuler">
                    </div>
                    ...
                    <div class="col-span-12 md:col-span-6">
                        ...
                        @if ($ex->photo)
                            <div class="mb-2"><img src="{{ asset('storage/' . $ex->photo) }}" alt="{{ $ex->name }}"
                                    class="max-w-[200px] h-auto"></div>
                        @endif
                    ...
                    <div class="col-span-12 text-right mt-4">
                        ...
                        <a href="{{ route('extrakulikuler.index') }}"
